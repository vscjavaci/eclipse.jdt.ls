 ...
 *
 * Code copied from org.eclipse.debug.core.model.IVariable
 *
 * Contributors:
 ...
 * Each variable has a value which may in turn contain more variables. A
 * variable may support value modification.
 * <p>
 * An implementation may choose to re-use or discard variables on iterative
 * thread suspensions. Clients cannot assume that variables are identical or
 * equal across iterative thread suspensions and must check for equality on
 * iterative suspensions if they wish to re-use the objects.
 * </p>
 * <p>
 * An implementation that preserves equality across iterative suspensions may
 * display more desirable behavior in some clients. For example, if variables
 * are preserved while stepping, a UI client would be able to update the UI
 * incrementally, rather than collapse and redraw the entire list or tree.
 * </p>
 * <p>
...
public interface IVariable extends IDebugElement, IValueModification {
     ...
     * event. Implementations may choose whether the last suspend event is the
     * last suspend event in this variable's debug target, or within the
     * thread(s) in which this variable is visible.
     * <p>
